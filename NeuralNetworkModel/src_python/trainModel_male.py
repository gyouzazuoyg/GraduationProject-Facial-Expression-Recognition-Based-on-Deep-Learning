...
def load_data_fromCSV_male():
    ...
    for i in range(datas.shape[0]):
        ...
        if(flags[i]=='Training'):
            if(np.argmax(gender_classfier.predict(gray_face)) == 1):
...
#加载数据
train_x,train_y,test_x,test_y = load_data_fromCSV_male()
...
if(IsTrain):
    ...
    model.fit(train_x, Y_train,#开始训练
                ...
                verbose=1)
    model.save('%s/saved_models/CNN_model_male.h5' %path_workspace)
else:
